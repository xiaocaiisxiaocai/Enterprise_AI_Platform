...
生成完整 Gate F/F.1 与本地摄取证据包（Release 构建、80 条回归、12 Golden、文档门禁；原子发布至 `artifacts\`）：
...
同步以规范化相对路径生成稳定 ID，以内容 SHA-256 生成版本；重复内容不提升 repository revision。源文件删除或变为无效 UTF-8、空文件、超限文件时，旧投影会从检索中撤出并返回隔离原因。当前仅在进程内保存状态，应用重启后重新扫描；它不包含恶意文件扫描、持久化 Checkpoint、正式审核或企业 Connector。
