...
API Trace 默认写入被 Git 忽略的 `.gate-f\search-traces.jsonl`。证据脚本重新执行构建、32 条回归（含 `REG-EVAL-*` Golden 契约负向样例）、12 个 Golden 用例和文档校验，并在 `artifacts\` 生成证据包、评测报告与评测 Trace；CI 为每个提交上传同结构 Artifact。评测报告不保存问题原文，只记录 Case ID、实际状态和引用。损坏数据集必须非零退出且不得产生伪造 Passed 报告。OIDC 等外部集成保持延期，直到获得企业权限和目标系统批准。
