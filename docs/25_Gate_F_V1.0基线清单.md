...
> 状态：**BaselineCandidate** ｜ 版本：**V1.0** ｜ 范围：单企业内部、本地确定性 Gate F 契约 ｜ 不代表：业务试点、生产发布或概率性 AI Evaluation

## 1. 基线目的

本清单定义 Gate F V1.0 可重复验证的最小安全边界。基线以 Git 注解标签 `V1.0` 指向的提交为唯一代码锚点；标签创建前保持 `BaselineCandidate`。标签不得移动、覆盖或复用，候选内容变化后必须重新执行全部门禁。
...
修改固定值、ACL/身份语义、拒答规则、Trace 哈希算法、Evidence schema、Golden 期望或批准快照会立即使候选基线失效。变更必须先增加失败回归，再修改实现，更新相关 ADR/测试计划，并重新生成 Evidence。未经用户明确授权，不得创建或推送 `V1.0` 标签。
