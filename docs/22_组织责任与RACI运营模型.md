...
下表是启动 Gate F 和 Gate P 所需的最小责任模型，不是新增编制。一个人可以承担多个责任帽子，但必须显式记录；企业级 15 角色目录用于后续展开专业责任，不能反向要求小团队先配置 15 名人员。

| Phase 0 责任帽子 | 合并的长期角色 | Phase 0 最终责任 | 真实映射 | 代理/升级 | 状态 |
|---|---|---|---|---|---|
| P0-BIZ | EXS、BUS、PRD、CHG | 业务问题、范围、价值口径和 Go/Hold/Stop | Open | Open | Open |
| P0-PLT | ARC、MOD、TOL、WFL | 最小架构、ADR、实现边界和技术交付 | Open | Open | Open |
| P0-TRUST | SEC、DAT、LEG | 身份、数据用途、权限、安全与合规硬门禁 | Open | Open | Open |
| P0-QUAL | KNO、EVA | 数据集、知识质量、评测方法和发布建议 | Open | Open | Open |
| P0-OPS | SRE、FIN | 环境、恢复、事件、支持容量和全量成本 | Open | Open | Open |

Gate F 开始前五个帽子必须映射到真实主体；映射的是责任，不要求全职投入。至少保持以下独立性：实现者不能是安全硬门禁的唯一批准者；评测对象的实现者不能是评测结果的唯一批准者；生产发布执行者不能自行豁免失败门禁。无法人员分离时，必须采用独立复核、外部评审、限时授权或只使用合成数据等补偿控制，并由 P0-BIZ 记录残余风险。
...
角色可由同一人员兼任，但不得违反第 5 节职责分离。组织规模不足时须记录补偿控制，例如独立复核、限时授权、抽样审计或外部评审。
...
- [ ] Phase 0 五个责任帽子映射真实主体，兼任和补偿控制已批准。
