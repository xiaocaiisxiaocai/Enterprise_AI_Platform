...
| ADR-012 | 单一企业 Tenant、组织 ACL 与权限交集模型 | Planned |
...
- `VER-ADR-004-001`：固定企业 Tenant 内至少两个不同组织 Group/主体的合成权限图中，未授权向量、详情和缓存命中均为 0；非配置 Tenant 同样拒绝；
...
## ADR-012：单一企业 Tenant、组织 ACL 与权限交集模型

**状态**：Planned

**背景**：一期服务单一企业内部用户。Agent 代人执行仍会跨越用户、组织 Group、资源 ACL、Tool 和数据边界；把部门建模成 Tenant 或只用共享服务账号都会造成权限语义错误。

**选项**：部门即 Tenant；完全移除 Tenant；固定企业 Tenant，并由用户授权、组织 Group、Agent Grant、Tool Scope、数据 ACL 和策略上下文取交集。

**决策**：一期固定一个企业 Tenant，由受信部署配置或企业 IdP Issuer 建立，客户端无权传入或切换。部门、项目和岗位是组织 Group/属性，不是 Tenant。所有实体、缓存、事件、Trace 和索引仍携带 `tenant_id`；人员、Service 和 Agent 使用不同 Principal。Agent 通过短期委托代表用户执行，有效权限取用户授权、组织 Group、Agent Grant、Tool Scope、数据 ACL 和上下文策略交集，高风险动作再加审批。

**后果**：避免把组织架构变化误当租户迁移，并保留明确的数据归属；但固定 `tenant_id` 不能被宣传为多租户能力。权限计算、缓存失效和撤权传播仍是关键依赖，失败时默认拒绝。

**验证与复审**：当前 Gate F PoC 已覆盖部门 ACL 预过滤、未知身份、非配置 Tenant、客户端 Tenant 注入、无证据拒答和引用定位的本地回归。正式验证仍需真实 IdP Group/源 ACL、撤权、离职、缓存污染和备份恢复测试。只有出现独立子公司治理、数据驻留或外部客户需求时，才启动多租户 ADR；迁移前不得复用部门字段充当 Tenant。

## ADR-013：Skill 学习采用受控供应链，不直接复制个人 Agent 模型

**状态**：Planned

**背景**：个人 Agent 项目展示了本地 Skill、Memory 和自我改进体验，但企业共享平台面向多个部门和高风险系统，不能让运行中 Agent 自行生成并发布代码或权限。
...
- 出现跨安全域/资源 ACL 事件、未经审批的高风险动作、SLO/RPO/RTO 连续不达标、许可证变化或上游停止维护时，立即触发相关 ADR 复审。
