...
- Phase 0 即建立租户隔离、默认拒绝、审计、Trace、预算与离线评测最小闭环，不得延后到功能开发完成后补做。
...
| 编号 | 必须满足的需求 | 验收证据 |
|---|---|---|
| FR-IAM-001 | 请求必须携带可验证的用户/服务身份、租户、会话与用途上下文；缺失或冲突时默认拒绝。 | 跨租户、伪造令牌、身份缺失和权限撤销测试。 |
...
|---|---|---|---|---|---|---|---|---|---|
| TR-FR-IAM-001 | BR-005 | FR-IAM-001 | `10_Governance_Security设计.md` §3 身份、会话与代理授权；§4 多租户隔离 | VER-FR-IAM-001-01 | Test | 身份缺失、跨租户、伪造令牌和撤权请求全部被拒绝 | —（未执行） | Security Owner | Planned |
...
| TBD-IAM-001 | P0 | Open | IdP、Tenant 定义、主体及源系统权限映射 | FR-IAM-001 | Security Architect | Security Owner、Data Owner | 身份/租户模型、权限矩阵和批准 ADR | 详细设计前 | — |
